        ...
        for (const auto &subgrid : subdivide(grid, m_max_cells_in_memory)) {
            ...
            GEOSSTRtree_query_r(
                ...
                [](void *hit, void *userdata) {
                    ...
                    auto vec =
                        static_cast<std::vector<const Feature *> *>(userdata);
            ...
            for (const auto &f : hits) {
                ...
                for (const auto &op : m_operations) {
                    ...
                    auto key = std::make_tuple(op->weights, op->values,
                                               op->coverage_threshold);
                    ...
                    // raster that have already been read. This may be possible
                    // when reading box is expanded slightly from floating-point
                    // roundoff problems.
                    ...
                    if (op->weighted()) {
                        ...
                        if (weights == nullptr) {
                            raster_values[op->weights] = op->weights->read_box(
