...
namespace exactextract {
    ...
    TEMPLATE_TEST_CASE("Unweighted stats", "[stats]", float, double, int) {
        ...
        auto g = GEOSGeom_read_r(
            context, "POLYGON ((0.5 0.5, 2.5 0.5, 2.5 2.5, 0.5 2.5, 0.5 0.5))");
        ...
        CHECK(stats.count() ==
              (0.25 + 0.5 + 0.25) + (0.50 + 1.0 + 0.50) + (0.25 + 0.0 + 0.25));

        CHECK(stats.sum() == (1 * 0.25 + 2 * 0.5 + 3 * 0.25) +
                                 (4 * 0.50 + 5 * 1.0 + 6 * 0.50) +
                                 (0 * 0.25 + 0 * 0.5 + 7 * 0.25));
        ...
        CHECK(stats.rawcount() ==
              5);  // 0.5 ,0.5 ,1.0 ,0.5 ,0.5 the rest are less than 0.3
    ...
    TEMPLATE_TEST_CASE("Weighted multiresolution stats", "[stats]", float,
        ...
        auto g = GEOSGeom_read_r(
            context, "POLYGON ((3.5 1.5, 6.5 1.5, 6.5 2.5, 3.5 2.5, 3.5 1.5))");

        Raster<float> areas =
            raster_cell_intersection(ex1.common_grid(ex2), context, g.get());
        ...
        std::valarray<double> cov_fracs = {0.25, 0.5, 0.5, 0.25,
                                           0.25, 0.5, 0.5, 0.25};

        CHECK(stats.weighted_mean() ==
              Approx((cov_values * cov_weights * cov_fracs).sum() /
                     (cov_weights * cov_fracs).sum()));
        CHECK(stats.mean() ==
              Approx((cov_values * cov_fracs).sum() / cov_fracs.sum()));

        CHECK(stats.weighted_fraction() ==
              Approx((cov_values * cov_weights * cov_fracs).sum() /
                     (cov_values * cov_fracs).sum()));
    }
    ...
    TEMPLATE_TEST_CASE("Missing data handling", "[stats]", float, double, int) {
        ...
        auto g = GEOSGeom_read_r(
            context, "POLYGON ((0.5 0.5, 1.5 0.5, 1.5 1.5, 0.5 1.5, 0.5 0.5))");
        ...
        Raster<TestType> all_values_missing{
            Matrix<TestType>{{{NA, NA}, {NA, NA}}}, extent};
        ...
        Raster<TestType> some_values_defined{
            Matrix<TestType>{{{1, 2}, {NA, NA}}}, extent};
    ...
    TEST_CASE("Stat subsets calculated for a specific category") {
        ...
        CHECK(stats.frac(1).value() ==
              Approx(stats.count(1).value() / stats.count()));
        CHECK(stats.frac(2).value() ==
              Approx(stats.count(2).value() / stats.count()));
        ...
        CHECK(stats.weighted_frac(1).value() ==
              Approx(stats.weighted_count(1).value() / stats.weighted_count()));
        CHECK(stats.weighted_frac(2).value() ==
              Approx(stats.weighted_count(2).value() / stats.weighted_count()));
    ...
    TEST_CASE("Unweighted stats consider all values when part of polygon is "
        ...
        auto g = GEOSGeom_read_r(
            context, "POLYGON ((0.5 0.5, 3.5 0.5, 3.5 3.5, 0.5 3.5, 0.5 0.5))");

        auto common_grid = values_grid.common_grid(weights_grid);

        Raster<float> areas =
            raster_cell_intersection(common_grid, context, g.get());
    ...
    TEST_CASE(
        "Variance calculations are correct for equally-weighted observations") {
    ...
    TEST_CASE("Variance calculations are correct for unequally-weighted "
              "observations") {
        std::vector<double> values{3.4,   2.9, 1.7,  8.8, -12.7,
                                   100.4, 8.4, 11.3, 50};
        std::vector<double> weights{1.0, 0.1, 1.0,  0.2, 0.44,
                                    0.3, 0.3, 0.83, 0};
        ...
        CHECK(wv.stdev() ==
              Approx(25.90092));  // output from Weighted.Desc.Stat::w.sd in R
        CHECK(wv.variance() ==
              Approx(670.8578));  // output from Weighted.Desc.Stat::w.var in R
        CHECK(wv.coefficent_of_variation() ==
              Approx(2.478301));  // output from Weighted.Desc.Stat::w.sd /
                                  // Weighted.Desc.Stat::w.mean
    }

    TEST_CASE("Variance calculations are correct for unequally-weighted "
              "observations, initial zeros") {
        ...
        CHECK(wv.stdev() ==
              Approx(0.7071068));  // output from Weighted.Desc.Stat::w.sd in R
        CHECK(wv.variance() ==
              Approx(0.5));  // output from Weighted.Desc.Stat::w.var in R
        CHECK(wv.coefficent_of_variation() ==
              Approx(0.7071068 / 8));  // output from Weighted.Desc.Stat::w.sd /
                                       // Weighted.Desc.Stat::w.mean
    }

    TEST_CASE("Weighted quantile calculations are correct for equally-weighted "
              "inputs") {
        std::vector<double> values{3.4, 2.9, 1.7, 8.8, -12.7, 100.4, 8.4, 11.3};
    ...
    TEST_CASE("Weighted quantile calculations are correct for "
              "unequally-weighted inputs") {
        std::vector<double> values{3.4,   2.9, 1.7,  8.8, -12.7,
                                   100.4, 8.4, 11.3, 50};
        std::vector<double> weights{1.0, 0.1, 1.0,  0.2, 0.44,
                                    0.3, 0.3, 0.83, 0};
        ...
        // check values against output of
        // wsim.distributions::stack_weighted_quantile in R
    ...
    TEST_CASE("Weighted quantile errors on invalid weights") {
        ...
        CHECK_THROWS(WeightedQuantiles().process(
